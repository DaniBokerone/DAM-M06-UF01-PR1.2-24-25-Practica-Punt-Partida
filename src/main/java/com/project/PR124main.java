...
public class PR124main {
    ...
    public void llistarEstudiantsFitxer() throws IOException {
        ...
        try (RandomAccessFile raf = new RandomAccessFile(fitxer, "r")) {
            raf.seek(0); // Comencem al principi del fitxer
            while (raf.getFilePointer() < raf.length()) {
    ...
    public void actualitzarNotaFitxer(int registre, float novaNota) throws IOException {
        ...
        try (RandomAccessFile raf = new RandomAccessFile(filePath, "rw")) {
            ...
            raf.seek(posicio + GRADE_POS); // Anem a la posició de la nota
    ...
    private void escriureNom(RandomAccessFile raf, String nom) throws IOException {
        // *************** CODI PRÀCTICA **********************/
        byte[] nomBytes = new byte[NAME_MAX_BYTES];
        byte[] nomEncoded = nom.getBytes(StandardCharsets.UTF_8);

        // Escriure el nom codificat, omplint amb zeros si és necessari
        System.arraycopy(nomEncoded, 0, nomBytes, 0, Math.min(nomEncoded.length, nomBytes.length));
        raf.write(nomBytes);
    }
}
